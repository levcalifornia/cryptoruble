...
std::string HelpMessageCli()
{
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Connect to JSON-RPC on <port> (default: %u or testnet: %u)"), 20181, 20181) + "\n";
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Cryptoruble Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  cryptoruble-cli [options] <command> [params]  " + _("Send command to Cryptoruble Core") + "\n" +
                  "  cryptoruble-cli [options] help                " + _("List commands") + "\n" +
                  "  cryptoruble-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
UniValue CallRPC(const string& strMethod, const UniValue& params)
{
    ...
    // cryptoruble:
    // we get cryptorubled output (possibly binary) of .write() that either:
