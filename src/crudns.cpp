/*
 * Simple DNS server for CryptoRuble project
 *
...
CruDns::CruDns(const char *bind_ip, uint16_t port_no,
    ...
    if(ret < 0) {
      throw runtime_error("CruDns::CruDns: Cannot create socket");
    } else {
      m_sockfd = ret;
    }
    ...
    if(bind(m_sockfd, (struct sockaddr *) &m_address,
      ...
      char buf[80];
      sprintf(buf, "CruDns::CruDns: Cannot bind to port %u", port_no);
    ...
    m_dap_ht  = (allowed_len && m_gw_suf_len)? (DNSAP*)calloc(CRUDNS_DAPSIZE, sizeof(DNSAP)) : NULL; 
    ...
    if(m_value == NULL) 
      throw runtime_error("CruDns::CruDns: Cannot allocate buffer");
    ...
    if(enums && *enums) {
      ...
      while(char *p_tok = strsep(&str, "|,"))
        if(*p_tok) {
	  if(m_verbose > 5)
	  LogPrintf("\tCruDns::CruDns: enumtrust=%s\n", p_tok);
	...
	if(c == '.' || c == '$') {
	  *p = 64;
	  if(p[1] > 040) { // if allowed domain is not empty - save it into ht
	    ...
	    m_allowed_qty++;
	    if(m_verbose > 3)
	      LogPrintf("\tCruDns::CruDns: Insert %s TLD=%s:%u\n", dnstype, p + 1, *p);
	  }
	  pos = step = 0;
	  continue;
	}
	...
	step |= 1;
	if(m_verbose > 3)
	  LogPrintf("\tCruDns::CruDns: Insert Local:[%s]->[%s] pos=%u step=%u\n", p, p_eq, pos, step);
	do 
	  pos += step;
        while(m_ht_offset[pos] != 0);
    ...
    if(m_verbose > 0)
	 LogPrintf("CruDns::CruDns: Created/Attached: %s:%u; Qty=%u:%u\n", 
    ...
    if(tollfree && *tollfree) {
      if(m_verbose > 3)
	LogPrintf("\tCruDns::CruDns: Setup deferred toll-free=%s\n", tollfree);
...
CruDns::~CruDns() {
    ...
    free(m_dap_ht);
    if(m_verbose > 0)
	 LogPrintf("CruDns::~CruDns: Destroyed OK\n");
} // CruDns::~CruDns


/*---------------------------------------------------*/

void CruDns::StatRun(void *p) {
  CruDns *obj = (CruDns*)p;
  obj->Run();
//cryptoruble  ExitThread(0);
} // CruDns::StatRun

/*---------------------------------------------------*/
void CruDns::Run() {
  if(m_verbose > 2) LogPrintf("CruDns::Run: started\n");
  ...
  if(m_verbose > 2) LogPrintf("CruDns::Run: Received Exit packet_len=%d\n", m_rcvlen);

} //  CruDns::Run

/*---------------------------------------------------*/

void CruDns::HandlePacket() {
  if(m_verbose > 2) LogPrintf("CruDns::HandlePacket: Handle packet_len=%d\n", m_rcvlen);
  ...
  if(m_verbose > 3) {
    LogPrintf("\tCruDns::HandlePacket: msgID  : %d\n", m_hdr->msgID);
    LogPrintf("\tCruDns::HandlePacket: Bits   : %04x\n", m_hdr->Bits);
    LogPrintf("\tCruDns::HandlePacket: QDCount: %d\n", m_hdr->QDCount);
    LogPrintf("\tCruDns::HandlePacket: ANCount: %d\n", m_hdr->ANCount);
    LogPrintf("\tCruDns::HandlePacket: NSCount: %d\n", m_hdr->NSCount);
    LogPrintf("\tCruDns::HandlePacket: ARCount: %d\n", m_hdr->ARCount);
  }
  ...
  do {
	...
	// Fill deferred toll-free default entries
        ...
        while(char *tf_fname = strsep(&tf_str, "|")) {
          if(m_verbose > 3)
	    LogPrintf("\tCruDns::HandlePacket: handle deferred toll-free=%s\n", tf_fname);
    ...
    for(uint16_t qno = 0; qno < m_hdr->QDCount && m_snd < m_bufend; qno++) {
      if(m_verbose > 5) 
        LogPrintf("\tCruDns::HandlePacket: qno=%u m_hdr->QDCount=%u\n", qno, m_hdr->QDCount);
...
uint16_t CruDns::HandleQuery() {
  ...
  *--key_end = 0; // Remove last dot, set EOLN
  ...
  if(m_verbose > 3) 
    LogPrintf("CruDns::HandleQuery: After TLD-suffix cut: [%s]\n", key);
  ...
  if(p) {
    ...
    if(m_allowed_qty) { // Activated TLD-filter
      if(*p != '.') {
        if(m_verbose > 3) 
      LogPrintf("CruDns::HandleQuery: TLD-suffix=[.%s] is not specified in given key=%s; return NXDOMAIN\n", p, key);
	return 3; // TLD-suffix is not specified, so NXDOMAIN
      ...
      do {
        ...
        if(m_ht_offset[pos] == 0) {
          if(m_verbose > 3) 
  	    LogPrintf("CruDns::HandleQuery: TLD-suffix=[.%s] in given key=%s is not allowed; return NXDOMAIN\n", p, key);
...
/*---------------------------------------------------*/
int CruDns::TryMakeref(uint16_t label_ref) {
  char val2[VAL_SIZE];
  char *tokens[MAX_TOK];
  int ttlqty = Tokenize("TTL", NULL, tokens, strcpy(val2, m_value));
  m_ttl = ttlqty? atoi(tokens[0]) : 24 * 3600;
  ...
  m_hdr->ANCount = 0;
  LogPrintf("CruDns::TryMakeref: Generated REF NS=%u\n", m_hdr->NSCount);
  return m_hdr->NSCount;
} //  CruDns::TryMakeref
/*---------------------------------------------------*/

int CruDns::Tokenize(const char *key, const char *sep2, char **tokens, char *buf) {
  int tokensN = 0;
...
void CruDns::Answer_ALL(uint16_t qtype, char *buf) {
  const char *key;
  switch(qtype) {
      case  1 : key = "A";      break;
      case  2 : key = "NS";     break;
  ...
  if(m_verbose > 0) LogPrintf("CruDns::Answer_ALL(QT=%d, key=%s); TokenQty=%d\n", qtype, key, tokQty);
  ...
  for(int tok_no = 0; tok_no < tokQty; tok_no++) {
      if(m_verbose > 1) 
	LogPrintf("\tCruDns::Answer_ALL: Token:%u=[%s]\n", tok_no, tokens[tok_no]);
      Out2(m_label_ref);
...
void CruDns::Fill_RD_IP(char *ipddrtxt, int af) {
  uint16_t out_sz;
  switch(af) {
...
void CruDns::Fill_RD_DName(char *txt, uint8_t mxsz, int8_t txtcor) {
  uint8_t *snd0 = m_snd;
...
} // CruDns::Fill_RD_DName

/*---------------------------------------------------*/
/*---------------------------------------------------*/

int CruDns::Search(uint8_t *key) {
  if(m_verbose > 1) 
    LogPrintf("CruDns::Search(%s)\n", key);
...
int CruDns::LocalSearch(const uint8_t *key, uint8_t pos, uint8_t step) {
  if(m_verbose > 1) 
    LogPrintf("CruDns::LocalSearch(%s, %u, %u) called\n", key, pos, step);
    do {
      pos += step;
      if(m_ht_offset[pos] == 0) {
        if(m_verbose > 3) 
  	  LogPrintf("CruDns::LocalSearch: Local key=[%s] not found; go to nameindex search\n", key);
...
DNSAP *CruDns::CheckDAP(uint32_t ip_addr) { 
  uint32_t hash = ip_addr * m_daprand;
  hash ^= hash >> 16;
  hash += hash >> 8;
  DNSAP *dap = m_dap_ht + (hash & (CRUDNS_DAPSIZE - 1));
  ...
  return (dap->ed_size <= CRUDNS_DAPTRESHOLD)? dap : NULL;
} // CruDns::CheckDAP 
...
int CruDns::SpfunENUM(uint8_t len, uint8_t **domain_start, uint8_t **domain_end) {
  int dom_length = domain_end - domain_start;
  const char *tld = (const char*)domain_end[-1];

  if(m_verbose > 3)
    LogPrintf("\tCruDns::SpfunENUM: Domain=[%s] N=%u TLD=[%s] Len=%u\n", 
  ...
  do {
    ...
    *pitut = 0; // EOLN at phone number end
    ...
    if(m_verbose > 3)
      LogPrintf("\tCruDns::SpfunENUM: ITU-T num=[%s]\n", itut_num);
    ...
    if(!m_verifiers.empty())
      for(int16_t qno = 0; qno >= 0; qno++) {
        ...
        if(m_verbose > 1) 
          LogPrintf("\tCruDns::SpfunENUM Search(%s)\n", q_str);
	...
	// bool matched = regex_search(string(itut_num), nameparts, tf->regex);
        if(m_verbose > 3) 
          LogPrintf("\tCruDns::SpfunENUM TF-match N=[%s] RE=[%s] -> %u\n", itut_num, tf->regex_str.c_str(), matched);
        if(matched)
...
void CruDns::Answer_ENUM(const char *q_str) {
...
void CruDns::HandleE2U(char *e2u) {
  ...
  *data++ = 0; // Remove '='
  ...
  if(sscanf(data, "%u | %u | %s", &ord, &pref, re) != 3)
    ...
    if(m_verbose > 3)
      LogPrintf("\tCruDns::HandleE2U: Parsed: %u %u %s %s\n", ord, pref, e2u, re);
...
/*---------------------------------------------------*/
bool CruDns::CheckEnumSig(const char *q_str, char *sig_str) {
    if(sig_str == NULL)
      return false;
